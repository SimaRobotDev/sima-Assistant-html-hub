/**
 * Apply verified bathroom MapVX patches into services-catalog.json.
 *
 * Safe by default (dry-run). Requires poiRef + lat + lng + validatedAt on each patch.
 *
 * Usage:
 *   node tools/apply-bathroom-mapvx-patches.mjs
 *   node tools/apply-bathroom-mapvx-patches.mjs --apply
 *   node tools/apply-bathroom-mapvx-patches.mjs --apply --only bano-pb-afex
 *   node tools/apply-bathroom-mapvx-patches.mjs --apply --jsonp
 */
...
const defaultPatchesPath = resolve(repoRoot, "data/bathroom-mapvx-patches.json");

function parseArgs(argv) {
  const args = { apply: false, jsonp: false, only: null, patches: defaultPatchesPath };
...
function main() {
  ...
  if (!existsSync(args.patches)) {
    ...
    console.error("Run: node tools/match-bathroom-catalog.mjs  (needs tools/.mapvx-local.json)");
    process.exit(1);
  }
  ...
  for (const entry of services) {
    if (entry.type !== "bathroom") continue;
  ...
  console.log("\nUpdated", catalogPath, "(" + changed + " bathroom(s))");
