/* AUTO-GENERATED from projects/cencomall/Assets/StreamingAssets/sima_services/data/services-catalog.json by tools/build-jsonp-assets.mjs. Do not edit by hand. */
window.__SERVICES_CATALOG__ = {
  "version": 1,
  "mall": "costanera",
  "locale": "es",
  "services": [
    {
      "id": "bano-pb-afex",
      "type": "bathroom",
      "sector": "afex",
      "name": "Baños — Sector Afex (Planta Baja)",
      "floors": [
        "PB"
      ],
      "features": {
        "mudador": true,
        "accesible": true,
        "hombresMujeres": true
      },
      "layout": "pasillo_lateral",
      "anchorStores": [
        {
          "brand": "AFEX",
          "local": "CC_PB_150",
          "role": "primary"
        },
        {
          "brand": "Audiomusica",
          "local": "CC_PB_204",
          "role": "secondary"
        },
        {
          "brand": "Easy",
          "local": "CC_PB_100",
          "role": "secondary"
        }
      ],
      "descriptions": {
        "short": "En el pasillo interior ubicado justo detrás de Afex.",
        "medium": "Pasillo lateral junto a Afex, cerca de Audiomusica y las escaleras mecánicas hacia Easy.",
        "long": "Ingresa por el pasillo interior; la referencia es Afex. En la entrada hay mudador y baño accesible."
      },
      "keywords": [
        "baño",
        "baños",
        "wc",
        "afex",
        "planta baja",
        "pb",
        "mudador",
        "accesible"
      ],
      "mapvx": {
        "placeId": "-N1jltFjKg6-2V7-RWxv",
        "placeIdType": "mudador",
        "poiRef": "-N1jltFjKg6-2V7-RWxv",
        "mapvxId": "-N1jltFjKg6-2V7-RWxv",
        "lat": -33.417478310948745,
        "lng": -70.605956800282,
        "validatedAt": "2026-07-21"
      }
    },
    {
      "id": "bano-n1-central",
      "type": "bathroom",
      "sector": "central",
      "name": "Baños — Pasillo central (Nivel 1)",
      "floors": [
        "1"
      ],
      "features": {
        "mudador": false,
        "accesible": false,
        "hombresMujeres": true
      },
      "layout": "isla_central",
      "anchorStores": [
        {
          "brand": "Piedras Australes",
          "local": "CC_N1_1196",
          "role": "primary"
        },
        {
          "brand": "Volka Chokolade",
          "local": "CC_N1_1198",
          "role": "primary"
        },
        {
          "brand": "Adagio Teas",
          "local": "CC_N1_1182",
          "role": "secondary"
        }
      ],
      "descriptions": {
        "short": "Frente a Piedras Australes y Volka, en la estructura central del pasillo.",
        "medium": "Bloque central del pasillo, frente a Piedras Australes y Volka, diagonal a Adagio Teas.",
        "long": "Estructura central del Nivel 1. Cruza el pasillo desde Piedras Australes o Volka."
      },
      "keywords": [
        "baño",
        "baños",
        "nivel 1",
        "piso 1",
        "central",
        "piedras austral",
        "volka"
      ],
      "mapvx": {
        "placeId": "-NDngnRTiuduXYbgJyCy",
        "placeIdType": "hombres",
        "poiRef": "-NDngnRTiuduXYbgJyCy",
        "mapvxId": "-NDngnRTiuduXYbgJyCy",
        "lat": -33.41733754716824,
        "lng": -70.60636214911938,
        "validatedAt": "2026-07-21"
      }
    },
    {
      "id": "bano-n2-guess",
      "type": "bathroom",
      "sector": "guess",
      "name": "Baños — Sector Guess (Nivel 2)",
      "floors": [
        "2"
      ],
      "features": {
        "mudador": false,
        "accesible": true,
        "hombresMujeres": true
      },
      "layout": "isla_central",
      "anchorStores": [
        {
          "brand": "Guess",
          "local": "CC_N2_2133",
          "role": "primary"
        }
      ],
      "descriptions": {
        "short": "En la estructura central frente a Guess y Longines.",
        "medium": "Isla central entre Guess y la fila Longines, Tous y Amphora.",
        "long": "Frente a Guess, la estructura del baño queda cruzando el pasillo principal."
      },
      "keywords": [
        "baño",
        "baños",
        "nivel 2",
        "guess",
        "central"
      ],
      "mapvx": {
        "placeId": "-ND5cG4BCEGNUPbShiUA",
        "placeIdType": "accesible",
        "poiRef": "-ND5cG4BCEGNUPbShiUA",
        "mapvxId": "-ND5cG4BCEGNUPbShiUA",
        "lat": -33.417938660088865,
        "lng": -70.60616668313742,
        "validatedAt": "2026-07-21"
      }
    },
    {
      "id": "bano-n2-ripley",
      "type": "bathroom",
      "sector": "ripley",
      "name": "Baños — Sector Ripley (Nivel 2)",
      "floors": [
        "2"
      ],
      "features": {
        "mudador": true,
        "accesible": true,
        "hombresMujeres": true
      },
      "layout": "pasillo_lateral",
      "anchorStores": [
        {
          "brand": "Ripley",
          "local": "CC_N2_1300",
          "role": "primary"
        }
      ],
      "descriptions": {
        "short": "En el pasillo lateral entre Ripley y Natura.",
        "medium": "Pasillo que separa Ripley de Natura y Black Bubba, junto al ascensor.",
        "long": "Pasillo lateral entre Ripley y Natura. Mudador al fondo del pasillo."
      },
      "keywords": [
        "baño",
        "baños",
        "nivel 2",
        "ripley",
        "mudador"
      ],
      "mapvx": {
        "placeId": "-NDeEvwA9XSQQTMmX-AR",
        "placeIdType": "mudador",
        "poiRef": "-NDeEvwA9XSQQTMmX-AR",
        "mapvxId": "-NDeEvwA9XSQQTMmX-AR",
        "lat": -33.417621872978934,
        "lng": -70.6070950627327,
        "validatedAt": "2026-07-21"
      }
    },
    {
      "id": "bano-n3-adidas",
      "type": "bathroom",
      "sector": "adidas",
      "name": "Baños — Sector Adidas Originals (Nivel 3)",
      "floors": [
        "3"
      ],
      "features": {
        "mudador": false,
        "accesible": true,
        "hombresMujeres": true
      },
      "layout": "isla_central",
      "anchorStores": [
        {
          "brand": "Adidas Originals",
          "local": "CC_N3_3129",
          "role": "primary"
        }
      ],
      "descriptions": {
        "short": "En la estructura central, entre Adidas Originals y New Man.",
        "medium": "Isla central entre Adidas Originals y Original Penguin, Weinbrenner y New Man.",
        "long": "Desde Adidas Originals, cruza el pasillo hacia la estructura central."
      },
      "keywords": [
        "baño",
        "baños",
        "nivel 3",
        "adidas",
        "central"
      ],
      "mapvx": {
        "placeId": "-NE7dF-IV3USqpdIsVJW",
        "placeIdNote": "sin corroborar contra el volcado MapVX; candidato -NE7dEXVfDMhDGlTpvka (bano_general_mujeres N3) — verificar en el mapa antes de rutear.",
        "poiRef": "-NE7dF-IV3USqpdIsVJW",
        "mapvxId": "-NE7dF-IV3USqpdIsVJW",
        "lat": -33.417958249359835,
        "lng": -70.60621429234743,
        "validatedAt": "2026-07-21"
      }
    },
    {
      "id": "bano-n3-ripley",
      "type": "bathroom",
      "sector": "ripley",
      "name": "Baños — Sector Ripley (Nivel 3)",
      "floors": [
        "3"
      ],
      "features": {
        "mudador": false,
        "accesible": true,
        "hombresMujeres": true
      },
      "layout": "pasillo_lateral",
      "anchorStores": [
        {
          "brand": "Ripley",
          "local": "CC_N3_1300",
          "role": "primary"
        },
        {
          "brand": "Wrangler",
          "local": "CC_N3_3204",
          "role": "secondary"
        }
      ],
      "descriptions": {
        "short": "En el pasillo lateral entre Ripley y Wrangler.",
        "medium": "Pasillo entre Ripley y Wrangler y Head, cerca del ascensor.",
        "long": "Entra al pasillo formado por Ripley y Wrangler. Baños a la izquierda."
      },
      "keywords": [
        "baño",
        "baños",
        "nivel 3",
        "ripley"
      ],
      "mapvx": {
        "placeId": "-NE7bc9sfcOYbhv_bC8w",
        "placeIdType": "mujeres",
        "poiRef": "-NE7bc9sfcOYbhv_bC8w",
        "mapvxId": "-NE7bc9sfcOYbhv_bC8w",
        "lat": -33.41766329044061,
        "lng": -70.6071349605918,
        "validatedAt": "2026-07-21"
      }
    },
    {
      "id": "bano-n4-under-armour",
      "type": "bathroom",
      "sector": "under-armour",
      "name": "Baños — Sector Under Armour (Nivel 4)",
      "floors": [
        "4"
      ],
      "features": {
        "mudador": false,
        "accesible": true,
        "hombresMujeres": true
      },
      "layout": "isla_central",
      "anchorStores": [
        {
          "brand": "Under Armour",
          "local": "CC_N4_4189",
          "role": "primary"
        },
        {
          "brand": "Hoka",
          "local": "CC_N4_4125",
          "role": "secondary"
        }
      ],
      "descriptions": {
        "short": "En la estructura central, entre Under Armour y Hoka.",
        "medium": "Isla central entre Under Armour y Hoka, Tramontina y Bamers.",
        "long": "Estructura central con escalera mecánica integrada. Cruza el pasillo desde Under Armour."
      },
      "keywords": [
        "baño",
        "baños",
        "nivel 4",
        "under armour",
        "central"
      ],
      "mapvx": {
        "placeId": "-ND_0dRJ45M2oQrUvCMX",
        "placeIdType": "accesible",
        "poiRef": "-ND_0dRJ45M2oQrUvCMX",
        "mapvxId": "-ND_0dRJ45M2oQrUvCMX",
        "lat": -33.418022334229725,
        "lng": -70.60622535645962,
        "validatedAt": "2026-07-21"
      }
    },
    {
      "id": "bano-n4-ripley",
      "type": "bathroom",
      "sector": "ripley",
      "name": "Baños — Sector Ripley (Nivel 4)",
      "floors": [
        "4"
      ],
      "features": {
        "mudador": true,
        "accesible": true,
        "hombresMujeres": true
      },
      "layout": "pasillo_lateral",
      "anchorStores": [
        {
          "brand": "Ripley",
          "local": "CC_N4_1300",
          "role": "primary"
        },
        {
          "brand": "Totto",
          "local": "CC_N4_4202",
          "role": "secondary"
        }
      ],
      "descriptions": {
        "short": "En el pasillo lateral entre Ripley y Totto.",
        "medium": "Pasillo entre Ripley y Totto, Weplay y Columbia. Mudador en la entrada.",
        "long": "Pasillo lateral Ripley–Totto. Mudador y ascensor en la esquina de Ripley."
      },
      "keywords": [
        "baño",
        "baños",
        "nivel 4",
        "ripley",
        "mudador"
      ],
      "mapvx": {
        "placeId": "-NE7iAG6k8loLvwgURDf",
        "placeIdType": "hombres",
        "poiRef": "-NE7iAG6k8loLvwgURDf",
        "mapvxId": "-NE7iAG6k8loLvwgURDf",
        "lat": -33.417691275200816,
        "lng": -70.60709841549397,
        "validatedAt": "2026-07-21"
      }
    },
    {
      "id": "bano-n5-comidas",
      "type": "bathroom",
      "sector": "patio-comidas",
      "name": "Baños — Patio de comidas (Nivel 5)",
      "floors": [
        "5"
      ],
      "features": {
        "mudador": false,
        "accesible": true,
        "hombresMujeres": true
      },
      "layout": "isla_central",
      "anchorStores": [
        {
          "brand": "Crepes y Waffles",
          "local": "CC_N5_5121",
          "role": "primary"
        },
        {
          "brand": "Doggis",
          "local": "CC_N5_5525",
          "role": "secondary"
        },
        {
          "brand": "Santa Brasa",
          "local": "CC_N5_5129",
          "role": "secondary"
        }
      ],
      "descriptions": {
        "short": "En la estructura central del patio de comidas.",
        "medium": "Isla central entre Crepes y Waffles y Buffet Express / Doggis.",
        "long": "Gran isla central del nivel de restaurantes, con acceso accesible hacia La Sanguchera del Barrio."
      },
      "keywords": [
        "baño",
        "baños",
        "nivel 5",
        "patio de comidas",
        "restaurantes",
        "comida"
      ],
      "mapvx": {
        "placeId": "-NE8I7MsWiQx0XEJGlDX",
        "placeIdType": "mujeres",
        "poiRef": "-NE8I7MsWiQx0XEJGlDX",
        "mapvxId": "-NE8I7MsWiQx0XEJGlDX",
        "lat": -33.41793390269381,
        "lng": -70.60629274696112,
        "validatedAt": "2026-07-21"
      }
    },
    {
      "id": "sala-lactancia-n2",
      "type": "nursing",
      "sector": "hm",
      "name": "Sala de lactancia (Nivel 2)",
      "floors": [
        "2"
      ],
      "features": {
        "accesible": true
      },
      "anchorStores": [
        {
          "brand": "GAP",
          "local": "CC_N2_2148",
          "role": "primary",
          "floors": [
            "2"
          ]
        },
        {
          "brand": "H&M",
          "local": "CC_N2_2136",
          "role": "secondary",
          "floors": [
            "2"
          ]
        }
      ],
      "descriptions": {
        "short": "Nivel 2, en el pasillo entre GAP y H&M.",
        "medium": "Sala de lactancia del Nivel 2, en el pasillo que conecta GAP con H&M.",
        "long": "Sala de lactancia del Nivel 2. Está en el pasillo que une GAP con H&M."
      },
      "keywords": [
        "sala de lactancia",
        "lactancia",
        "lactario",
        "amamantar",
        "dar de mamar",
        "dar pecho",
        "nursing",
        "nursing room",
        "breastfeeding",
        "nivel 2",
        "piso 2",
        "gap",
        "h&m",
        "hm"
      ],
      "mapvx": {
        "placeId": "-NDeXfqsAvnjX7VToDag",
        "placeIdSource": "mapvx-directory",
        "validatedAt": "2026-09-10"
      }
    },
    {
      "id": "mudador-pb-afex",
      "type": "changing_table",
      "sector": "afex",
      "name": "Mudadores — Planta Baja",
      "floors": [
        "PB"
      ],
      "features": {
        "accesible": true
      },
      "anchorStores": [
        {
          "brand": "AFEX",
          "local": "CC_PB_150",
          "role": "primary",
          "floors": [
            "PB"
          ]
        }
      ],
      "descriptions": {
        "short": "Planta Baja, en el pasillo interior detrás de Afex.",
        "medium": "Planta Baja, en el pasillo interior detrás de Afex, junto a los baños del sector.",
        "long": "Planta Baja, en el pasillo interior detrás de Afex, junto a los baños del sector."
      },
      "keywords": [
        "mudador",
        "mudadores",
        "cambiador",
        "cambiadores",
        "cambiar al bebe",
        "changing table",
        "panal",
        "panales",
        "bebe",
        "planta baja",
        "afex"
      ],
      "mapvx": {
        "placeId": "-N1jltFjKg6-2V7-RWxv",
        "placeIdSource": "mapvx-directory",
        "validatedAt": "2026-09-10"
      }
    },
    {
      "id": "mudador-pb-2",
      "type": "changing_table",
      "sector": "ripley",
      "name": "Mudadores — Planta Baja",
      "floors": [
        "PB"
      ],
      "features": {
        "accesible": true
      },
      "anchorStores": [
        {
          "brand": "Ripley",
          "local": "CC_N1_1300",
          "role": "primary",
          "floors": [
            "PB"
          ]
        }
      ],
      "descriptions": {
        "short": "Planta Baja, al lado de Ripley.",
        "medium": "Planta Baja, al lado de Ripley.",
        "long": "Planta Baja, al lado de Ripley."
      },
      "keywords": [
        "mudador",
        "mudadores",
        "cambiador",
        "cambiadores",
        "cambiar al bebe",
        "changing table",
        "panal",
        "panales",
        "bebe",
        "planta baja",
        "ripley"
      ],
      "mapvx": {
        "placeId": "-NGTN_kdH0MTrCtpaDGB",
        "placeIdSource": "mapvx-directory",
        "validatedAt": "2026-09-10"
      }
    },
    {
      "id": "mudador-n2-ripley",
      "type": "changing_table",
      "sector": "ripley",
      "name": "Mudadores — Nivel 2",
      "floors": [
        "2"
      ],
      "features": {
        "accesible": true
      },
      "anchorStores": [
        {
          "brand": "Ripley",
          "local": "CC_N2_1300",
          "role": "primary",
          "floors": [
            "2"
          ]
        }
      ],
      "descriptions": {
        "short": "Nivel 2, al lado de Ripley.",
        "medium": "Nivel 2, al lado de Ripley.",
        "long": "Nivel 2, al lado de Ripley."
      },
      "keywords": [
        "mudador",
        "mudadores",
        "cambiador",
        "cambiadores",
        "cambiar al bebe",
        "changing table",
        "panal",
        "panales",
        "bebe",
        "nivel 2",
        "piso 2",
        "ripley"
      ],
      "mapvx": {
        "placeId": "-NDeEvwA9XSQQTMmX-AR",
        "placeIdSource": "mapvx-directory",
        "validatedAt": "2026-09-10"
      }
    },
    {
      "id": "mudador-n3",
      "type": "changing_table",
      "sector": "ripley",
      "name": "Mudadores — Nivel 3",
      "floors": [
        "3"
      ],
      "features": {
        "accesible": true
      },
      "anchorStores": [
        {
          "brand": "Ripley",
          "local": "CC_N3_1300",
          "role": "primary",
          "floors": [
            "3"
          ]
        }
      ],
      "descriptions": {
        "short": "Nivel 3, al lado de Ripley.",
        "medium": "Nivel 3, al lado de Ripley.",
        "long": "Nivel 3, al lado de Ripley."
      },
      "keywords": [
        "mudador",
        "mudadores",
        "cambiador",
        "cambiadores",
        "cambiar al bebe",
        "changing table",
        "panal",
        "panales",
        "bebe",
        "nivel 3",
        "piso 3",
        "ripley"
      ],
      "mapvx": {
        "placeId": "-NDe_Xg61b8E4IombD1q",
        "placeIdSource": "mapvx-directory",
        "validatedAt": "2026-09-10"
      }
    },
    {
      "id": "mudador-n4",
      "type": "changing_table",
      "sector": "ripley",
      "name": "Mudadores — Nivel 4",
      "floors": [
        "4"
      ],
      "features": {
        "accesible": true
      },
      "anchorStores": [
        {
          "brand": "Ripley",
          "local": "CC_N4_1300",
          "role": "primary",
          "floors": [
            "4"
          ]
        }
      ],
      "descriptions": {
        "short": "Nivel 4, al lado de Ripley.",
        "medium": "Nivel 4, al lado de Ripley.",
        "long": "Nivel 4, al lado de Ripley."
      },
      "keywords": [
        "mudador",
        "mudadores",
        "cambiador",
        "cambiadores",
        "cambiar al bebe",
        "changing table",
        "panal",
        "panales",
        "bebe",
        "nivel 4",
        "piso 4",
        "ripley"
      ],
      "mapvx": {
        "placeId": "-ND_0Yi_bYgm2EM7jctQ",
        "placeIdSource": "mapvx-directory",
        "validatedAt": "2026-09-10"
      }
    },
    {
      "id": "ascensor-ripley",
      "type": "elevator",
      "sector": "ripley",
      "name": "Ascensores · Ripley",
      "floors": [
        "PB",
        "1",
        "2",
        "3",
        "4",
        "5"
      ],
      "features": {
        "accesible": true
      },
      "layout": "banco_vertical",
      "routeApproach": [
        {
          "local": "CC_N2_2188",
          "query": "Natura Cosméticos",
          "floors": [
            "2"
          ],
          "weight": 0.38
        }
      ],
      "anchorStores": [
        {
          "brand": "Easy",
          "local": "CC_PB_100",
          "role": "secondary",
          "floors": [
            "PB"
          ]
        },
        {
          "brand": "JUMBO",
          "local": "CC_N1_1000",
          "role": "secondary",
          "floors": [
            "1"
          ]
        },
        {
          "brand": "Ripley",
          "local": "CC_N2_1300",
          "role": "primary",
          "floors": [
            "2"
          ]
        },
        {
          "brand": "Natura Cosméticos",
          "local": "CC_N2_2188",
          "role": "secondary",
          "floors": [
            "2"
          ]
        },
        {
          "brand": "Nike Rise",
          "local": "CC_N2_2192",
          "role": "secondary",
          "floors": [
            "2"
          ]
        },
        {
          "brand": "BOLD",
          "local": "CC_N3_3208",
          "role": "secondary",
          "floors": [
            "3"
          ]
        },
        {
          "brand": "Ripley",
          "local": "CC_N3_1300",
          "role": "secondary",
          "floors": [
            "3"
          ]
        },
        {
          "brand": "Sparta",
          "local": "CC_N4_4208",
          "role": "secondary",
          "floors": [
            "4"
          ]
        },
        {
          "brand": "Ripley",
          "local": "CC_N4_1300",
          "role": "secondary",
          "floors": [
            "4"
          ]
        },
        {
          "brand": "Burger King",
          "local": "CC_N5_5524",
          "role": "secondary",
          "floors": [
            "5"
          ]
        }
      ],
      "descriptions": {
        "short": "Eje Ripley / ancla, todos los pisos.",
        "medium": "Zona Ripley: Easy (PB), Jumbo (N1), Nike/Ripley (N2), Bold (N3), Sparta (N4), Burger King (N5).",
        "long": "Torre del sector Ripley. En cada piso la referencia es la tienda ancla de ese nivel (Easy, Jumbo, Ripley, Bold, Sparta o Burger King)."
      },
      "keywords": [
        "ascensor",
        "ascensores",
        "elevador",
        "elevator",
        "ripley",
        "easy",
        "jumbo",
        "natura",
        "nike",
        "nike rise",
        "bold",
        "sparta",
        "burger king",
        "bk",
        "ancla"
      ],
      "mapvx": {
        "placeId": "-N1e2dagzGnuGIFpinw0",
        "placeIdNote": "el volcado MapVX lo lista como 'Ascensores Torre Andrés Bello' (acceso de estacionamiento, N-1). Confirmar que sea el banco de pasajeros del eje Ripley y un destino de ruta válido en route-view-totems.",
        "poiRef": "-N1e2dagzGnuGIFpinw0",
        "mapvxId": "-N1e2dagzGnuGIFpinw0",
        "lat": -33.41780349399868,
        "lng": -70.6073646247387,
        "validatedAt": "2026-07-21"
      }
    },
    {
      "id": "ascensor-vitacura",
      "type": "elevator",
      "sector": "vitacura",
      "name": "Ascensores · Av. Vitacura",
      "floors": [
        "PB",
        "1",
        "2",
        "3",
        "4",
        "5"
      ],
      "features": {
        "accesible": true
      },
      "layout": "banco_vertical",
      "routeApproach": [
        {
          "query": "Cajero Automático",
          "floors": [
            "1"
          ],
          "weight": 1
        }
      ],
      "anchorStores": [
        {
          "brand": "Decathlon",
          "local": "CC_PB_300",
          "role": "primary",
          "floors": [
            "PB"
          ]
        },
        {
          "brand": "Maxi K",
          "local": "CC_N1_1116",
          "role": "primary",
          "floors": [
            "1"
          ]
        },
        {
          "brand": "Victorinox",
          "local": "CC_N1_1118",
          "role": "secondary",
          "floors": [
            "1"
          ]
        },
        {
          "brand": "The Body Shop",
          "local": "CC_N1_1122",
          "role": "secondary",
          "floors": [
            "1"
          ]
        },
        {
          "brand": "Victoria's Secret",
          "local": "CC_N2_2128",
          "role": "secondary",
          "floors": [
            "2"
          ]
        },
        {
          "brand": "Samsonite",
          "local": "CC_N3_3120",
          "role": "secondary",
          "floors": [
            "3"
          ]
        },
        {
          "brand": "Studio F",
          "local": "CC_N3_3124",
          "role": "secondary",
          "floors": [
            "3"
          ]
        },
        {
          "brand": "OAKLEY",
          "local": "CC_N4_4120",
          "role": "secondary",
          "floors": [
            "4"
          ]
        },
        {
          "brand": "Maui And Sons",
          "local": "CC_N4_4124",
          "role": "secondary",
          "floors": [
            "4"
          ]
        },
        {
          "brand": "Barra Chalaca",
          "local": "CC_N5_5116",
          "role": "secondary",
          "floors": [
            "5"
          ]
        }
      ],
      "descriptions": {
        "short": "Costado Av. Vitacura (Decathlon / Maxi K).",
        "medium": "Lado Vitacura: Decathlon (PB), Maxi K–Victorinox (N1), Victoria’s Secret (N2), Samsonite/Studio F (N3), Oakley (N4), Barra Chalaca (N5).",
        "long": "Ascensores del costado Av. Vitacura. En N1 quedan junto a Maxi K / escaleras, entre Victorinox y The Body Shop."
      },
      "keywords": [
        "ascensor",
        "ascensores",
        "elevador",
        "elevator",
        "vitacura",
        "decathlon",
        "maxik",
        "maxi k",
        "maxx",
        "victorinox",
        "body shop",
        "the body shop",
        "victoria secret",
        "moule",
        "samsonite",
        "studio f",
        "oakley",
        "maui",
        "barra chalaca"
      ],
      "mapvx": {
        "placeId": "node:993809113",
        "placeIdNote": "identificador propio — el volcado MapVX no expone los ascensores de pasajeros. Formato node: — validar como destino de ruta en route-view-totems o reemplazar por el -Nxxx equivalente.",
        "poiRef": "node:993809113",
        "mapvxId": "node:993809113",
        "lat": -33.41820479394959,
        "lng": -70.60576099902391,
        "validatedAt": "2026-07-21"
      }
    },
    {
      "id": "ascensor-hm",
      "type": "elevator",
      "sector": "hm",
      "name": "Ascensores · H&M",
      "floors": [
        "PB",
        "1",
        "2",
        "3",
        "4",
        "5"
      ],
      "features": {
        "accesible": true
      },
      "layout": "banco_vertical",
      "anchorStores": [
        {
          "brand": "AFEX",
          "local": "CC_PB_150",
          "role": "secondary",
          "floors": [
            "PB"
          ]
        },
        {
          "brand": "Blush-Bar",
          "local": "CC_N1_1170",
          "role": "secondary",
          "floors": [
            "1"
          ]
        },
        {
          "brand": "H&M",
          "local": "CC_N2_2136",
          "role": "primary",
          "floors": [
            "2"
          ]
        },
        {
          "brand": "H&M",
          "local": "CC_N3_2136",
          "role": "secondary",
          "floors": [
            "3"
          ]
        },
        {
          "brand": "Fabrics",
          "local": "CC_N4_4148",
          "role": "secondary",
          "floors": [
            "4"
          ]
        },
        {
          "brand": "Texas Ribs",
          "local": "CC_N5_5144",
          "role": "secondary",
          "floors": [
            "5"
          ]
        }
      ],
      "descriptions": {
        "short": "Pasillo lateral, zona H&M.",
        "medium": "Pasillo H&M: Afex (PB), Blush Bar (N1), H&M (N2–N3), Fabrics (N4), Texas Ribs (N5).",
        "long": "Ascensores del pasillo lateral (lado H&M), opuesto a Vitacura."
      },
      "keywords": [
        "ascensor",
        "ascensores",
        "elevador",
        "elevator",
        "h&m",
        "hm",
        "blush",
        "blush bar",
        "fabrics",
        "texas ribs",
        "afex",
        "lateral"
      ],
      "mapvx": {
        "placeId": "-N2SSgpaCHzZe80nRkOx",
        "placeIdNote": "el volcado MapVX lo lista como 'Ascensores Torre Tobalaba' (acceso de estacionamiento, N-1). Confirmar que sea el banco de pasajeros del lado H&M y un destino de ruta válido.",
        "poiRef": "-N2SSgpaCHzZe80nRkOx",
        "mapvxId": "-N2SSgpaCHzZe80nRkOx",
        "lat": -33.41743185612792,
        "lng": -70.60591321438551,
        "validatedAt": "2026-07-21"
      }
    },
    {
      "id": "ascensor-zara",
      "type": "elevator",
      "sector": "zara",
      "name": "Ascensores · Zara",
      "floors": [
        "3"
      ],
      "features": {
        "accesible": true
      },
      "layout": "banco_vertical",
      "anchorStores": [
        {
          "brand": "Hugo",
          "local": "CC_N3_3180",
          "role": "primary",
          "floors": [
            "3"
          ]
        },
        {
          "brand": "Zara",
          "local": "CC_N3_2156",
          "role": "secondary",
          "floors": [
            "3"
          ]
        }
      ],
      "descriptions": {
        "short": "Pasillo junto a Hugo (Nivel 3).",
        "medium": "Banco en el pasillo lateral junto a Hugo, al costado de Zara (Nivel 3) — no es el de H&M.",
        "long": "En el Nivel 3, ascensores del pasillo junto a Hugo (node:19041 / 19042)."
      },
      "keywords": [
        "ascensor",
        "ascensores",
        "elevador",
        "elevator",
        "zara",
        "hugo",
        "nivel 3",
        "piso 3"
      ],
      "mapvx": {
        "placeId": "node:19041",
        "placeIdNote": "identificador propio (node:19041 / 19042). Formato node: — validar como destino de ruta en route-view-totems o reemplazar por el -Nxxx equivalente.",
        "poiRef": "node:19041",
        "mapvxId": "node:19041",
        "lat": -33.41725506172419,
        "lng": -70.60655797133222,
        "validatedAt": "2026-07-22"
      }
    },
    {
      "id": "customer-service-n1",
      "type": "customer_service",
      "sector": "sony-dbs-maconline",
      "name": "Servicio al cliente (Nivel 1)",
      "floors": [
        "1"
      ],
      "features": {},
      "layout": "modulo_central",
      "anchorStores": [
        {
          "brand": "Sony",
          "local": "CC_N1_1154",
          "role": "primary"
        },
        {
          "brand": "Dbs Beauty Store",
          "local": "CC_N1_1158",
          "role": "secondary"
        },
        {
          "brand": "MacOnline",
          "local": "CC_N1_1124",
          "role": "secondary"
        }
      ],
      "routeApproach": [
        {
          "query": "Servicio al cliente",
          "local": "CC_N1_1154",
          "floors": [
            "1"
          ],
          "weight": 1.2,
          "maxMeters": 45
        }
      ],
      "descriptions": {
        "short": "Módulo de servicio al cliente en Nivel 1.",
        "medium": "Entre Dbs Beauty, Sony y MacOnline, en el pasillo central del Nivel 1.",
        "long": "En el Nivel 1, módulo de atención / servicio al cliente ubicado en el pasillo entre Dbs Beauty Store, Sony y MacOnline."
      },
      "keywords": [
        "atencion al cliente",
        "atención al cliente",
        "servicio al cliente",
        "customer service",
        "sac",
        "informacion",
        "información",
        "modulo",
        "nivel 1",
        "sony",
        "dbs",
        "maconline"
      ],
      "mapvx": {
        "placeId": "-NjtsXycJ45NbmQu_hbx",
        "placeIdSource": "mapvx-context",
        "searchQueries": [
          "Servicio al cliente",
          "Atención al cliente",
          "Atencion al cliente"
        ]
      }
    },
    {
      "id": "cowork-n2",
      "type": "cowork",
      "sector": "bubba-steve-madden",
      "name": "Cowork (Nivel 2)",
      "floors": [
        "2"
      ],
      "features": {},
      "layout": "local_pasillo",
      "anchorStores": [
        {
          "brand": "Bubba Essentials",
          "local": "CC_N2_2168",
          "role": "primary"
        },
        {
          "brand": "Steve Madden",
          "local": "CC_N2_2160",
          "role": "secondary"
        }
      ],
      "routeApproach": [
        {
          "query": "Cowork",
          "local": "CC_N2_2168",
          "floors": [
            "2"
          ],
          "weight": 1.2,
          "maxMeters": 40
        }
      ],
      "descriptions": {
        "short": "Espacio Cowork en Nivel 2.",
        "medium": "Entre Bubba y Steve Madden, en el pasillo del Nivel 2.",
        "long": "En el Nivel 2, espacio Cowork ubicado entre Bubba Essentials y Steve Madden, frente a las escaleras mecánicas del pasillo."
      },
      "keywords": [
        "cowork",
        "co work",
        "co-work",
        "espacio de trabajo",
        "trabajo",
        "oficina",
        "workspace",
        "nivel 2",
        "bubba",
        "steve madden",
        "madden"
      ],
      "mapvx": {
        "placeId": "-NjrEJiRPmJXjhupLASS",
        "placeIdSource": "mapvx-context",
        "searchQueries": [
          "Cowork",
          "Co work",
          "Co-work"
        ]
      }
    }
  ]
};
